...
async def read_products(db: Session = Depends(depends.get_db)):
    product = crud.get_products(db)
    return product

#creating the new products
@app.post("/products", response_model=schemas.Product, tags= ["PRODUCT"])
def create_products(product : schemas.ProductCreate, db : Session = Depends(depends.get_db)):
    return crud.create_products(db=db, product=product)

#read the cartitems
@app.get("/carts", response_model= list[schemas.CartItem], tags= ["CART"])
def read_cart(db: Session = Depends(depends.get_db), current_user: schemas.User = Depends(depends.get_current_user)):
